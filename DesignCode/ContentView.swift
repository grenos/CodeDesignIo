...
//  CodeDesignIo
//
//  Created by Vasileios Gkreen on 03/05/2020.
	...
	var body: some View {
		...
		ZStack {
			...
			BottomCardView(show: $showCard)
				...
				.gesture(
					DragGesture().onChanged { drag in
						self.bottomState = drag.translation
						if self.showFull {
					...
					.onEnded { _ in
						// if we drag card too low dismiss it
						if self.bottomState.height > 50 {
							self.showCard = false
						}
						if (self.bottomState.height < -100 && !self.showFull) ||
							(self.bottomState.height < -240 && self.showFull) {
							self.bottomState.height = -300
							self.showFull = true
						} else {
...
/*
	Animation on this binding is first controlled by the parrent view (ContentView)
	Then this view (BottomCardView) binds the parent view binding with the binding of (RingView)
	and the final animation of the ring is handled by the RingView.
*/
struct BottomCardView: View {
	...
	var body: some View {
		VStack(spacing: 20) {
			...
			Text("This certificate is proof that Vasilis Green was awarded the UI design course with aproval from a Design+Code instructor")
